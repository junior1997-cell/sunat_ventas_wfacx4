...
function guardaryeditarnproveedor(e) {
  ...
  $.ajax({
    ...
    success: function (datos) {
      sw_success("¡Éxito!", datos, 5000);
      $.post("../ajax/compra.php?op=selectProveedor", function(r) {
        $("#idproveedor").html(r); $("#idproveedor").val(datos); // Establecer el valor seleccionado        
      });
    },
    error: function (jqXHR, textStatus, errorThrown) {
      toastr_error("¡Error!", "Hubo un error al procesar la solicitud.", 700 );      
    },
    complete: function () {
      limpiar();
...
function guardaryeditar(e) {
  ...
  for (var i = 0; i <cant.length; i++) {
    ...
    if (inpC.value==0 || inpC.value=="" ){  sw=sw+1; }
  }

  if(sw!=0){
    toastr_error('Revisar cantidad!', 'Por favor revise la cantidad de los productos.' );        
    inpP.focus();
  } else {
    Swal.fire({
...
function mostrar(idcompra) {
  ...
  $.post("../ajax/compra.php?op=listarDetalle&id="+idcompra,function(r){ $("#detalles").html(r); });
}
...
function agregarDetalle(idarticulo,familia,codigo_proveedor,codigo,nombre, precio_factura,stock,umedidacompra, precio_unitario, valor_unitario, factorc, nombreum) {
  ...
  if (subarticulooption=='0') {
    ...
    setTimeout(function(){ document.getElementById("codigob").focus(); },500);
  }
}
...
function agregarDetalleBarra(e) {
  ...
  if(e.keyCode===13  && !e.shiftKey) {
    ...
    if (codigob != null) {
      ...
      for (var i = 0; i < id.length; i++) { var idA=id[i];  var cantiS=can[i]; }//Fin while
