...
function init() {
  ...
  $("#formulario").on("submit", function (e) { regcompras(e); });

  // ══════════════════════════════════════ I N I T I A L I Z E   S E L E C T 2 ══════════════════════════════════════
  $("#filtro_tcomprobante").select2({ dropdownParent: $('.card-header'), theme: "bootstrap4", placeholder: "Seleccione", allowClear: true, });
  $("#tmonedaa").select2({ dropdownParent: $('.card-header'), theme: "bootstrap4", placeholder: "Seleccione", allowClear: true, });
  ...
  $.post("../ajax/inventarios.php?op=selectAlm", function (r) { $("#almacenlista").html(r); });
  $.post("../ajax/articulo.php?op=comboarticulokardex", function (r) { $("#codigoInterno").html(r); });

  regventas();
  regvenagruxdia('TODOS', 'TODOS', 'TODOS', 'PEN');
}
...
//- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
//              R E P O R T E  D E  V E N T A S 
//- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -

function regvenagruxdia(fechadel, fechaal, tcomprobante, tipo_moneda) {
  ...
  tabla = $('#tbllistadoVentas').dataTable({
    ...
    buttons: [
      ...
      { extend: 'copyHtml5', exportOptions: { columns: [0, 1, 2, 3, 4, 5, 6, 7], }, text: `<i class="fas fa-copy" data-toggle="tooltip" data-original-title="Copiar"></i>`, className: "btn bg-gradient-gray m-r-5px", footer: true, },
      { extend: 'excelHtml5', exportOptions: { columns: [0, 1, 2, 3, 4, 5, 6, 7], }, text: `<i class="far fa-file-excel fa-lg" data-toggle="tooltip" data-original-title="Excel"></i>`, className: "btn bg-gradient-success m-r-5px", footer: true, },
      { extend: 'pdfHtml5', exportOptions: { columns: [0, 1, 2, 3, 4, 5, 6, 7], }, text: `<i class="far fa-file-pdf fa-lg" data-toggle="tooltip" data-original-title="PDF"></i>`, className: "btn bg-gradient-danger m-r-5px", footer: false, orientation: 'landscape', pageSize: 'LEGAL', },
    ...
    "autoWidth": true,
    "ajax": {
      "url": '../ajax/ajaxReporteVenta.php?action=ListarReporteMesyDia&fechadel=' + fechadel + '&fechaal=' + fechaal + '&tcomprobante=' + tcomprobante + '&idempresa=' + $idempresa + '&tipo_moneda=' + tipo_moneda,
      "type": "get",
      "dataType": "json",
    ...
    "columnDefs": [
      {
        "targets": 8,  // Esta es la posición de la columna "estado", que parece ser la 8ª columna (empezando desde 0)
        "render": renderEstado
      }
    ],
    language: {
    ...
    "footerCallback": function (row, data, start, end, display) {
      ...
      $(api.column(7).footer()).html(totalGeneral.toFixed(2));
...
//- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
//          F I N  R E P O R T E  D E  V E N T A S 
//- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
...
function fetchReport(ano, mes, tmonedaa) {
  let url = `https://wfacx.com/sistema/ajax/ajaxReporteVenta.php?action=ListarReporteMes&ano=${ano}&mes=${mes}&idempresa=1&tmon=${tmonedaa}`;
...
function cargando_search() {
  var fechadel = $('#filtro_fechadel').val();
  var fechaal = $('#filtro_fechaal').val();
  var tcomprobante = $('#filtro_tcomprobante').find(':selected').text();
  var tipo_moneda = $('#tmonedaa').find(':selected').text();

  if ($('#filtro_fechadel').val() == '' || $('#filtro_fechadel').val() == null) { fechadel = ""; }
  if ($('#filtro_fechaal').val() == '' || $('#filtro_fechaal').val() == null) { fechaal = "" }
  if ($('#filtro_tcomprobante').val() == '' || $('#filtro_tcomprobante').val() == null) { tcomprobante = "" }
  if ($('#tipo_moneda').val() == '' || $('#tipo_moneda').val() == null) { tipo_moneda = "" }
  $('.cargando').show().html(`<i class="fas fa-spinner fa-pulse fa-sm"></i> Buscando ${fechadel} ${fechaal} ${tcomprobante} ${tipo_moneda}...`);
}
function filtros() {
  ...
  if (fechadel == '' || fechadel == 0 || fechadel == null) { fechadel = ""; fechadel = ""; }
  if (fechaal == '' || fechaal == null || fechaal == 0) { fechaal = ""; fechaal = "" }
  if (tcomprobante == '' || tcomprobante == null || tcomprobante == 0) { tcomprobante = ""; tcomprobante = "" }
  if (tipo_moneda == '' || tipo_moneda == null || tipo_moneda == 0) { tipo_moneda = ""; tipo_moneda = "" }

  $('.cargando').show().html(`<i class="fas fa-spinner fa-pulse fa-sm"></i> Buscando ${fechadel} ${fechaal} ${tcomprobante} ${tipo_moneda}...`);

  regvenagruxdia(fechadel, fechaal, tcomprobante, tipo_moneda);
}
