...
class PDF_Invoice extends FPDF
{
  ...
  function numCotizacion($num, $ruc)
  {
    $r1  = $this->w - 70;
    $r2  = $r1 + 60;
    $y1  = 15;
    ...
    $this->SetFont("Arial", "B", 11);
    $this->Cell(10, 5, utf8_decode("COTIZACIÓN DE VENTA"), 0, 0, "C");
  ...
  // Client address
  function addClientAdresse($fecha, $cliente, $num_documento, $domicilio, $estado, $usuario, $tcotizacion, $moneda, $nrofactura)
  {
    $r1     = $this->w - 200;
    $r2     = $r1 + 68;
    $y1     = 28;
    ...
    $this->MultiCell(250, 4, utf8_decode("Fecha Emisión        : ") . $fecha);
    ...
    $this->SetXY(50, $y1 + 23);
    $this->MultiCell(150, 3, utf8_decode("Factura: ") . $nrofactura);

    $this->SetXY(100, $y1 + 23);
    $this->MultiCell(150, 3, utf8_decode("Moneda: ") . $moneda);

    $this->SetXY(150, $y1 + 23);
    $this->MultiCell(150, 3, utf8_decode("Tipo de cotización: ") . $tcotizacion);
  ...
  function addClientAdresse2($fecha, $cliente, $num_documento, $domicilio, $estado, $usuario, $guia)
  {
    ...
    $this->MultiCell(150, 3, utf8_decode("Moneda     : ") . $guia);
  ...
  function addCols($tab)
  {
    ...
    $r2  = $this->w - ($r1 * 2);
    $y1  = 54;
    $y2  = $this->h - 70 - $y1; // para el largo del cuadro del primer detalle
    $this->SetXY($r1, $y1);
...
*/
  ...
  function observSunat($nro, $std, $observacion, $webconsul, $nresolucion)
  {
    if ($std == '5') {
      $esta = 'aceptada';
    } elseif ($std == '3') {
    ...
    $this->SetFont("Arial", "", 8);
    $this->SetXY($r1 + 2, $y1);
    $this->MultiCell(120, 4, utf8_decode("La cotización número, ") . $nro . " ha sido " . $esta . ".");

    $this->SetFont("Arial", "", 7);
    $this->SetXY($r1 + 2, $y1 + 3);
    $this->MultiCell(120, 4, utf8_decode("Observaciones: "));
    $this->MultiCell(116, 3, utf8_decode($observacion));
  }
  ...
  function numerocuentas($banco1, $cuenta1, $banco2, $cuenta2, $banco3, $cuenta3, $banco4, $cuenta4, $cuentacci1, $cuentacci2, $cuentacci3, $cuentacci4)
  {
    ...
    $this->SetXY($r1 + 2, $y1);
    $this->MultiCell(116, 3, "Cuentas bancarias: ");

    $this->MultiCell(116, 4, utf8_decode($banco1 . ": " . $cuenta1 . " CCI:  " . $cuentacci1));
    $this->MultiCell(116, 4, utf8_decode($banco2 . ": " . $cuenta2 . " CCI:  " . $cuentacci2));
    $this->MultiCell(116, 4, utf8_decode($banco3 . ": " . $cuenta3 . " CCI:  " . $cuentacci3));
    $this->MultiCell(116, 4, utf8_decode($banco4 . ": " . $cuenta4 . " CCI:  " . $cuentacci4));
  }
  ...
  function tipocambio($tcambio, $conversion)
  {


    $r1  = $this->w - 78; // movimiento de lados
    $r2  = $r1 + 30; //Ancho de cuadro
    $y1  = $this->h - 69;
    $y2  = $y1 + 22;
    $this->RoundedRect($r1, $y1, ($r2 - $r1), ($y2 - $y1), 2.5, 'D');
    ...
    $this->SetFont("Arial", "B", 8);
    $this->SetXY(135, $y1 + 5);
    $this->Cell(20, 4, "T. CAMBIO " . $tcambio, 0, 0, "C");
    $this->SetXY(130, $y1 + 10);
    $this->Cell(20, 4, "TOTAL S/", 0, 0, "C");
    $this->SetFont("Arial", "B", 13);
    $this->SetXY(132, $y1 + 15);
    $this->Cell(20, 4, $conversion, 0, 0, 'C');
  }


  function addCadreEurosFrancs($impuesto, $nombretrib, $moneda)
  {
    ...
    $r1  = $this->w - 45; // movimiento de lados
    $r2  = $r1 + 35; //Ancho de cuadro
    $y1  = $this->h - 69;
    $y2  = $y1 + 22;
    $this->RoundedRect($r1, $y1, ($r2 - $r1), ($y2 - $y1), 2.5, 'D');
    $this->Line($r1 + 16,  $y1, $r1 + 16, $y2); // LINEA CENTRAL 
    ...
    $this->SetXY(163, $y1 + 13);
    $this->Cell(20, 4, "I.G.V.", 0, 0, "C");

    $this->SetXY(163, $y1 + 17);
    $this->Cell(20, 4, "Total", 0, 0, "C");
  }


  function addTVAs($igv, $total, $moneda, $tdescuento)
  {
    $this->SetFont('Arial', '', 9);

    $re  = $this->w - 20; //MOvimiento lados
    $rf  = $this->w - 29;
    $y1  = $this->h - 69;
    $this->SetFont("Arial", "B", 7);
    $this->SetXY($re, $y1 + 5);
    //$this->Cell( 17,4, $moneda.sprintf("%0.2F", $total-($total*$igv/($igv+100))), '', '', 'R');
    $this->Cell(4, 4, number_format($total - $igv, 2), '', '', 'R');

    $this->SetXY($re, $y1 + 9);
    //$this->Cell( 17,4, $moneda.sprintf("%0.2F", $igv), '', '', 'R');
    $this->Cell(4, 4, number_format($tdescuento, 2), '', '', 'R');
    ...
    $this->Cell(4, 4, number_format($total, 2), '', '', 'R');
  }
