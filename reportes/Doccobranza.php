...
function addSociete( $nom, $adresse,$logo,$ext_logo )
{
    ...
    $this->Image($logo , 10 ,6, 25 , 20 , $ext_logo);
    $this->SetXY( $x1, $y1 );
...
function numdoccobranza( $num, $ruc )
{
    $r1  = $this->w - 80;
    $r2  = $r1 + 70;
    $y1  = 6;
    ...
    $this->SetFont( "Arial", "B", 11);
    $this->Cell(10,5, utf8_decode("DOCUMENTO DE COBRANZA"), 0, 0, "C");
...
// Client address
function addClientAdresse($fecha,$cliente,$num_documento,$domicilio,$estado,$usuario,$tcotizacion, $condición, $moneda)
{
    $r1     = $this->w - 190;
    $r2     = $r1 + 68;
    $y1     = 24;
    ...
    $this->SetXY(70, $y1+25);
    $this->MultiCell( 150, 3, utf8_decode("Tipo  : ").$tcotizacion);

	$this->SetXY(120, $y1+25);
    $this->MultiCell( 150, 3, utf8_decode("Condición  : ").$condición);

    $this->SetXY(175, $y1+25);
...
function addCols( $tab )
{
    ...
    $r2  = $this->w - ($r1 * 2) ;
    $y1  = 54;
    $y2  = $this->h - 58 - $y1; // para el largo del cuadro del primer detalle
    $this->SetXY( $r1, $y1 );
...
function observSunat($nro, $std, $hs, $webconsul, $nresolucion)
{
    ...
    $this->MultiCell(120,4, utf8_decode("El documento de cobranza número, ").$nro." ha sido ".$esta.".");


    $this->SetTextColor(194,8,8);
    $this->SetFont( "Arial", "b", 14);
    $this->SetXY( $r1+2, $y1+20);
    $this->MultiCell(200,4, utf8_decode("NO VALIDO PARA CRÉDITO FISCAL, SOLO PRA FINES DE COBRANZA."));
...
function addCadreEurosFrancs($impuesto, $nombretrib)
{
    ...
    $this->SetXY( $r1, $y1+5 );
    $this->Cell(20,4, "NETA", 0, 0, "C");

    $this->SetXY( $r1, $y1+9 );
    $this->Cell(20,4, "INPUESTOS ", 0, 0, "C");

    $this->SetXY( $r1, $y1+13 );
    $this->Cell(20,4, "OTROS", 0, 0, "C");
...
//                  "qte"     => qte,
//                  "tva"     => code_tva );
function addTVAs( $igv, $total, $neta, $otros )
{
    ...
    $this->Cell( 1,4, number_format($neta,2), '', '', 'R');
    ...
    $this->Cell( 1,4, number_format($otros,2), '', '', 'R');
