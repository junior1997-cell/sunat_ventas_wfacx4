    ...
    while ($reg = $rspta2->fetch_object()) {
      ...
      if (is_null($reg)) {
        ...
        if ($transac == "COMPRA") {
          ...
          $pdf->SetFont('Arial', '', 8);
          $pdf->Row(array(utf8_decode($fecha), '', utf8_decode($tipodoc), '', utf8_decode($docum), '', utf8_decode($transac), '', utf8_decode($cantidad), '', $costo1, '', utf8_decode($unimed), '|', number_format($saldofin, 2), '', number_format($costo2, 2), '', number_format($valorfin_p, 2)));
          $sw = $sw + 1;
          ...
          $pdf->SetFont('Arial', '', 8);
          $pdf->Row(array(utf8_decode($fecha), '', utf8_decode($tipodoc), '', utf8_decode($docum), '', utf8_decode($transac), '', utf8_decode($cantidad), '', $costo1, '', utf8_decode($unimed), '|', number_format($saldofinal_nc, 2), '', number_format($costo2, 2), '', number_format($valorfin_p, 2)));
