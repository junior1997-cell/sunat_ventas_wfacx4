...
function numBoleta( $num , $ruc)
{
    ...
    $this->SetFont( "Arial", "B", 11);
    $this->Cell(10,5, utf8_decode("NOTA DE PEDIDO"), 0, 0, "C");
...
// Client address
function addClientAdresse($fecha,$cliente,$domicilio,$num_documento,$estado,$usuario, $guia)
{
    $r1     = $this->w - 200;
    $r2     = $r1 + 68;
    $y1     = 26;
 ...
 // Client address
function addClientAdresse2($fecha,$cliente,$domicilio,$num_documento,$estado,$usuario, $guia)
{
    $r1     = $this->w - 200;
    $r2     = $r1 + 68;
    $y1     = 170; // Edicion
...
function observSunat($nro, $std, $hs, $webconsul, $nresolucion, $recibos)
{
    $this->SetFont( "Arial", "B", 7);
    $r1  = 10;
    $r2  = $r1 + 120;
    ...
    $this->Cell(10,3, utf8_decode("Observaciones: "));
    ...
    $this->MultiCell(120,4, utf8_decode("La nota de pedido número, ").$nro." ha sido creada");


    // $this->SetFont( "Arial", "B", 7);
    // $this->SetXY( $r1+2, $y1+7);
    // $this->MultiCell(116,3, utf8_decode("Recibos pendientes de cliente: ".$recibos));
}



function observSunat2($nro, $std, $hs, $webconsul, $nresolucion, $recibos)
{
    $this->SetFont( "Arial", "B", 7);
    $r1  = 10;
    $r2  = $r1 + 120;
...
function addCadreEurosFrancs()
{
    ...
    $this->SetFont( "Arial", "B", 10);
    $this->SetXY( $r1, $y1+10 );
    $this->Cell(35,4, "IMPORTE TOTAL", 0, 0, "C");
}
...
//                  "qte"     => qte,
//                  "tva"     => code_tva );
function addTVAs($total, $moneda, $totalfinal)
{
    $this->SetFont('Arial','',8);
    ...
    $totalf=0;
    $totalf=$total + $totalfinal;
    $this->SetXY( $re, $y1+9 );
    $this->Cell( 10,4, $moneda.sprintf("%0.2F", $totalf), '', '', 'R');
     
}

function addTVAs2($total, $moneda, $totalfinal)
{
    $this->SetFont('Arial','',8);
