...
function numoServicio( $num, $ruc )
{
    ...
    $this->SetFont( "Arial", "B", 11);
    $this->Cell(10,5, utf8_decode("ORDEN DE SERVICIO"), 0, 0, "C");
...
// Client address
function addProveeAdresse($fecha, $fechaentrega, $cliente,$num_documento,$domicilio,$estado)
{
    $r1     = $this->w - 190;
    $r2     = $r1 + 68;
    $y1     = 24;
    ...
    $this->SetXY( 140, $y1+2);
    $this->SetFont( "Arial", "", 8);
    $this->MultiCell( 60, 4, utf8_decode("Fecha entrega         : ").$fechaentrega);

    $this->SetXY( $r1, $y1+6);
    $this->SetFont( "Arial", "", 8);
    $this->MultiCell( 150, 3,utf8_decode("Proveedor   : ").$cliente);
...
function addCols( $tab )
{
    ...
    $r2  = $this->w - ($r1 * 2) ;
    $y1  = 54;
    $y2  = $this->h - 60 - $y1; // para el largo del cuadro del primer detalle
    $this->SetXY( $r1, $y1 );
...
function observ($nro, $std)
{
    ...
    $this->MultiCell(120,4, utf8_decode("La orden de servicio número, ").$nro." ha sido ".$esta.".");

    $this->SetFont( "Arial", "", 7);
    $this->SetXY( $r1+2, $y1+7);
    $this->MultiCell(116,3, utf8_decode("------- "));
}
    ...
    function observSunatComprobante($nro, $std, $hs, $tipodoc, $webconsul)
{
    ...
    $this->SetFont( "Arial", "", 8);
    $this->SetXY( $r1+2, $y1+3);
    $this->MultiCell(125,5, utf8_decode("La ".utf8_decode($comp)." número, ").$nro." ha sido ".$esta.".");

   $this->SetFont( "Arial", "", 7);
    $this->SetXY( $r1+2, $y1+7);
    $this->MultiCell(116,3, utf8_decode("Autorizado a ser emisor electrónico mediante R.I. SUNAT N° 00000. Representación impresa de la factura electrónica. Puede verificar su clave SOL. Código de seguridad(Hash): ".trim($hs). "     Consulte su comprobante en ". $webconsul));
}
    


function observSunatNC($nro, $std, $hs)
{
    $this->SetFont( "Arial", "B", 7);
    $r1  = 10;
    $r2  = $r1 + 120;
    ...
    $this->SetXY( $r1+2, $y1+7);
    $this->MultiCell(100,3, utf8_decode("Autorizado a ser emisor electrónico mediante RS SUNAT N° 155-2017. Representación impresa de la nota de crédito electrónica. Puede verificar usando su clave SOL. Código de seguridad(Hash): ".trim($hs)));
}


function observSunatNC2($nro, $std, $hs)
{
    $this->SetFont( "Arial", "B", 7);
    $r1  = 10;
    $r2  = $r1 + 120;
...
function observSunatND($nro, $std, $hs)
{
    ...
    $this->SetFont( "Arial", "", 6);
    $this->SetXY( $r1+2, $y1+7);
    $this->MultiCell(100,3, utf8_decode("Autorizado a ser emisor electrónico mediante R.I. SUNAT N° 00000. Representación impresa de la factura electrónica. Puede verificar su clave SOL. Código de seguridad(Hash): ".trim($hs)));
}
...
function addTVAsNC( $igv, $total, $moneda )
{
    ...
    $this->SetXY( $re, $y1-25 );
    $this->Cell( 5,4, $moneda.sprintf("%0.2F", $total - $igv), '', '', 'R');
    $this->SetXY( $re, $y1-20 );
    $this->Cell( 4,4, $moneda.sprintf("%0.2F", $igv), '', '', 'R');
    $this->SetXY( $re, $y1-14 );
...
function addTVAsNC2( $igv, $total, $moneda )
{
    ...
    $this->SetXY( $re, $y1+10 );
    $this->Cell( 5,4, $moneda.sprintf("%0.2F", $igv), '', '', 'R');
