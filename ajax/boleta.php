...
switch ($_GET["op"]) {
  ...
  case 'listarDetalle':
    ...
    $igv = 0;
    $total = 0;
    echo '<thead style="background-color:#A9D0F9"> <th>Artículo</th> <th>Cantidad</th> <th>Precio Venta</th> <th>Subtotal</th> </thead>';
  ...
  case 'selectClienteDocumento':
    ...
    $persona = new Persona();
    $rspta = $persona->listarC();
    while ($reg = $rspta->fetch_object()) { echo '<option value=' . $reg->idpersona . '>' . $reg->num_documento . '</option>'; }
  break;
  ...
  case 'selectSerie':
    ...
    while ($reg = $rspta->fetch_object()) { echo '<option value=' . $reg->idnumeracion . '>' . $reg->serie . '</option>'; }
  break;
  ...
  case 'llenarNumeroFactura':
    ...
    while ($reg = $rspta->fetch_object()) {  echo $reg->addnumero;  }
  break;
  ...
  case 'listar':
    ...
    while ($reg = $rspta->fetch_object()) {
      ...
      $data[] = array(
        "0" =>'<div class="dropdown">
          ...
          <ul class="dropdown-menu pull-center">
            <li>
              ...
              <i class="fa fa-level-down"  data-toggle="tooltip" title="Dar de baja" ></i> Dar de baja</a>
            </i>
            <li>
              <a onclick="duplicarb(' . $reg->idboleta . ')"  style="color:green;"  data-toggle="tooltip" title="Duplicar boleta" ' . $stt . '">
                <i  class="fa fa-files-o"></i>Duplicar boleta
              </a>
            </li>
            <li>
              <a  onclick="prea42copias2(' . $reg->idboleta . ')">
                <i class="fa  fa-print"  data-toggle="tooltip" title="Imprimir formato 2 copias" onclick=""></i> Imprimir formato 2 copias
              </a>
            <li>
              <a  onclick="preticket2(' . $reg->idboleta . ')"><i class="fa  fa-print"  data-toggle="tooltip" title="Imprimir Ticket"> </i> Imprimir Ticket </a>
            </li>
            <li>
              <a onclick="prea4completo2(' . $reg->idboleta . ')"><i class="fa  fa-print"  data-toggle="tooltip" title="Imprimir formato completo"> </i>Imprimir formato completo</a>
            </li>
            <li>
              <a onclick="enviarcorreo(' . $reg->idboleta . ')"><i class="fa  fa-send"  data-toggle="tooltip" title=""> </i>Enviar por correo</a>
            </li>
          </ul>
        </div>',
        "1" => $reg->fecha,
        ...
        "6" => ($reg->tarjetadc == '1') ? '<img src="../files/articulos/tarjetadc.png" width="20px" data-toggle="tooltip" title="TARJETA ' . $reg->montotarjetadc . '">' : '',
  ...
  case 'envioautomatico':
    ...
    while ($reg = $rspta->fetch_object()) {
      ...
      if ($reg->estado == '3') { $stt = 'disabled'; $sunat = ''; }
      if ($reg->estado == '0') { $stt = 'none'; $sunat = ''; }
      $estadoenvio = '1';
  ...
  case 'autonumeracion':
    ...
    while ($reg = $rspta->fetch_object()) { echo $reg->Nnumero; }
  break;
  ...
  case 'listarDR':
    ...
    while ($reg = $rspta->fetch_object()) {
      $data[] = array(
        ...
        "6" => $reg->total,
        "7" => $reg->fechabaja,
        "8" => ($reg->estado == '0')  ? '<i style="color:#BA4A00;"  > <span>NOTA</span></i>' : '<i  style="color:#E59866;" > <span>BAJA</span></i>',
        "9" => $reg->vendedorsitio,
        "10" => ($reg->estado == '0') ? '<button class="btn btn-warning"  onclick="ConsultaDR(' . $reg->idboleta . ')"> <i class="fa fa-eye" data-toggle="tooltip" title="Ver documento" ></i> </button>' : ''
      );
    }
    $results = array(
      "sEcho" => 1, //Información para el datatables
