<?php
	...
	switch ($_GET["op"]) {
		...
		case 'guardar_y_editar_articulo':		
			...
			if (empty($idarticulo)) {
				...
				echo $rspta ? $rspta : "Artículo no se pudo crear";
			} else {
				...
				echo $rspta ? $rspta : "Artículo no se pudo actualizar";
			}		
							...
							<div class="dropdown">
								<button type="button" class="btn btn-primary btn-sm dropdown-toggle" data-bs-toggle="dropdown" aria-haspopup="true" aria-expanded="false"><i class="fas fa-cog"></i> </button>
									<div class="dropdown-menu" style="">
										...
										<button class="dropdown-item" onclick="desactivar(' . $reg->idarticulo . ')" ><i class="fas fa-skull-crossbones text-danger"></i> Desactivar</button>
									</div>
							</div>
						</div> ' :'
						<div class="btn-group mb-1">
							<div class="dropdown">
								...
								<div class="dropdown-menu" style="">
										<a class="dropdown-item" href="' . $url . $reg->codigo . '&st=' . $reg->st2 . '&pr=' . $reg->precio . '" target="_blank"><i class="fas fa-barcode"></i> Código de barra</a>
										<button class="dropdown-item" onclick="mostrar(' . $reg->idarticulo . ')" ><i class="fas fa-pencil-alt"></i> Editar</button>
										<button class="dropdown-item" onclick="activar(' . $reg->idarticulo . ')" ><i class="fas fa-check text-success"></i> Activar</button>
								</div>
							</div>
						</div>'
					,
					...
					"0" => ($reg->estado) ? '<div class="btn-group mb-1">
						<div class="dropdown">
							...
							<div class="dropdown-menu" style="">
								...
								<a class="dropdown-item" onclick="desactivar(' . $reg->idarticulo . ')" ><i class="fas fa-skull-crossbones text-danger"></i> Desactivar</a>
							</div>
						</div>
						...
						<div class="dropdown">
							...
							<div class="dropdown-menu" style="">
								<a class="dropdown-item" onclick="mostrar(' . $reg->idarticulo . ')" ><i class="fas fa-pencil-alt"></i> Editar </a>
								<a class="dropdown-item" onclick="activar(' . $reg->idarticulo . ')" ><i class="fas fa-check text-success"></i> Activar </a>
							</div>
						</div>
					</div>',
					"1" => '<div class="d-flex align-items-center"> 
		...
		case 'importar_articulo':

			$tmpName = $_FILES["upload_file_articulo"]["tmp_name"];
			$error = $_FILES["upload_file_articulo"]["error"];
			...
			for ($i = 2; $i <= $numerofila; $i++) {
				...
				if ($codigo != "") {
					...
					$articulo->insertarArticulosMasivo(
