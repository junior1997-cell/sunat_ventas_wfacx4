...
switch ($_GET["op"]){
	case 'guardaryeditar':
		if (empty($idmesa)){
			$rspta = $marca->insertarMarca($nromesa);
			echo $rspta ? "Mesa registrada" : "Mesa no se pudo registrar";
		}
		else {
			$rspta = $marca->editarMarca($idmesa,$nromesa);
