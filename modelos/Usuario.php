...
class Usuario
{
  ...
  public function __construct( $id_usr_sesion = 0, $id_empresa_sesion = 0 )
  {
	...
	{
		$sql = "INSERT into usuario (nombre, apellidos, tipo_documento, num_documento, direccion, telefono, email, cargo, login, clave, imagen, user_created)
    values ('$nombre','$apellidos', '$tipo_documento', '$num_documento', '$direccion', '$telefono', '$email','$cargo', '$login', '$clave', '$imagen', '$this->id_usr_sesion')";

		$idusuarionew = ejecutarConsulta_retornarID($sql);

		// Insertar en vendedorsitio SOLO si cargo es igual a 1 (Ventas)
		if ($cargo == 1) {
			$sql_vendedor = "INSERT into vendedorsitio(nombre, estado, idempresa, user_created) values ('$nombre', 1, $this->id_empresa_sesion, '$this->id_usr_sesion')";
	...
	{
		$sql = "UPDATE usuario set nombre='$nombre', apellidos='$apellidos',tipo_documento='$tipo_documento', num_documento='$num_documento', direccion='$direccion', 
	...
	public function verificar($login, $clave, $empresa)	{
		...
		$sql = "SELECT u.idusuario, u.nombre, u.tipo_documento, u.num_documento, u.telefono, u.email, u.cargo, u.imagen, u.login, e.nombre_razon_social, e.idempresa, 
		co.igv,	e.nombre_comercial,	e.numero_ruc,	e.domicilio_fiscal  
		from usuario u 
