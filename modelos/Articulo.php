...
class Articulo
{
  ...
  public function filtros_table() {    
    $sql = "SELECT f.idfamilia, f.descripcion, f.estado, COUNT(a.idarticulo) as cant
    FROM familia as f
    ...
    return $retorno = array(
			...
			'filtro_marca' 			=> $familia , 
		);  

  }

  //Implementar un método para listar los registros
  public function listar($idempresa, $idfamilia) {
    ...
    if ($idfamilia == 'todos') {
      ...
      $tipo_familia = "AND f.idfamilia = '$idfamilia'";
    }
    ...
    a.ccontable, a.stock as st2, a.marca, um.nombreum, date_format(a.fechavencimiento, '%d/%m/%Y') as fechavencimiento, al.nombre as nombreal
    from articulo a 
    inner join familia f on a.idfamilia=f.idfamilia 
    ...
    where not a.nombre='1000ncdg' and e.idempresa='$idempresa' $tipo_familia and al.estado='1'";
    return ejecutarConsulta($sql);
  }
  ...
  public function buscararticulo($key) {
    ...
    if ($result->num_rows > 0) {
      ...
      while ($row = $result->fetch_assoc()) {
        ...
        $html .= '<div><a class="suggest-element"  codigo="' . utf8_encode($row['codigo']) . '"  
        ...
        mticbperu="' . $row['mticbperu'] . ' ">' . utf8_encode($row['nombre']) . '</a></div>';
      }
    }
    echo $html;
  }
  ...
  public function comboarticulo($anor, $alma) {
    $sql = "SELECT  a.idarticulo,  a.codigo,  a.nombre, 
  ...
  public function costoGlobal($ano, $mes) {
    ...
    where a.tipoitem NOT IN('servicios') and a.idarticulo not in('1') and year(c.fecha)='$ano' and month(c.fecha)='$mes' group by codigo";
    return $costounitario = ejecutarConsulta($promcosto);
  }

  public function obtenerdatos($ano, $mes, $codigoarticulo) {
    $promcosto = "SELECT *  from articulo where idarticulo='$codigoarticulo'";
    return $costounitario = ejecutarConsulta($promcosto);
  }

  public function obtenerdatosmargeng($ano, $mes) {
    $promcosto = "SELECT a.codigo, a.nombre, sum(mg.totalventas) as totalventas, sum(mg.totalcompras) as totalcompras, format((sum(mg.totalventas) - sum(mg.totalcompras)) / sum(mg.totalventas) ,2) as ganancia, 
    format(((sum(mg.totalventas) - sum(mg.totalcompras))/ sum(mg.totalventas)) * 100,2) as porcentaje 
    from  margenganancia mg 
    inner join articulo a on mg.idarticulo=a.idarticulo 
    where  mg.ano='$ano' and mg.mes='$mes' group by a.nombre ";
    return $costounitario = ejecutarConsulta($promcosto);
  }

  public function obtenerdatosmargengindividual($ano, $mes) {
    $promcosto = "SELECT a.codigo, a.nombre, mg.totalventas, mg.totalcompras, mg.ganancia, mg.porcentaje 
    from  margenganancia mg 
    inner join articulo a on mg.idarticulo=a.idarticulo ";
    return $costounitario = ejecutarConsulta($promcosto);
  }
  ...
  public function promedioboleta($ano, $mes, $codigoarticulo)  {
    ...
    inner join boleta b on b.idboleta=db.idboleta 
    where a.idarticulo='$codigoarticulo' and year(b.fecha_emision_01)='$ano' and month(b.fecha_emision_01)='$mes'";
    return $promediobol = ejecutarConsulta($promboleta);
  }

  public function promediofabotodos($ano, $mes) {
    $promboleta = "SELECT codigo, nombre, sum(promedio) as promedio 
    from (select a.codigo, a.nombre, format(sum(db.valor_uni_item_31)/count(db.valor_uni_item_31), 2) as promedio 
    from detalle_boleta_producto db 
    inner join articulo a on db.idarticulo=a.idarticulo 
    inner join boleta b on b.idboleta=db.idboleta 
    where a.idarticulo not in('1') and a.tipoitem not in ('servicios') and year(b.fecha_emision_01)='$ano' and month(b.fecha_emision_01)='$mes' group by codigo
    union all 
    select a.codigo, a.nombre, format(sum(df.valor_uni_item_14)/count(df.valor_uni_item_14), 2) as promedio 
    ...
    where a.idarticulo not in('1') and a.tipoitem not in ('servicios') and year(f.fecha_emision_01)='$ano' and month(f.fecha_emision_01)='$mes' group by codigo) 
  ...
  public function promediofactura($ano, $mes, $codigoarticulo) {
    $promfactura = "SELECT format(sum(df.valor_uni_item_14)/count(df.valor_uni_item_14), 2) as promedio
    from detalle_fac_art df inner join articulo a on df.idarticulo=a.idarticulo 
    inner join factura f on f.idfactura=df.idfactura 
    where a.idarticulo='$codigoarticulo' and year(f.fecha_emision_01)='$ano' and month(f.fecha_emision_01)='$mes'";
    return $promediofac = ejecutarConsulta($promfactura);
  }

  public function promediofacturatodos($ano, $mes, $codigoarticulo) {

    $promfactura = "SELECT  a.codigo,  format(sum(df.valor_uni_item_14)/count(df.valor_uni_item_14), 2) as promedio
        from detalle_fac_art df inner 
        join articulo a on df.idarticulo=a.idarticulo 
        inner join factura f on f.idfactura=df.idfactura 
        where 
